...
const (
	...
	DefaultLabelActiveBorderThickness = unit.Dp(2)
)
...
type DropDown struct {
	...
	// LabelActiveBorderThickness is the thickness of the border of
	// active (under focus) dropdown
	LabelActiveBorderThickness unit.Dp
}

// NewDropdown creates a new DropDown.
func NewDropdown(ddValues []string, hint string, selectedMenuItemHandler MenuItemHandlerType) *DropDown {
	ddWidget := NewDropDownWidget(ddValues, hint, DefaultOffsetX, DefaultOffsetY, DefaultDropdownTextSize, selectedMenuItemHandler)
	return &DropDown{
		...
		LabelActiveBorderThickness: DefaultLabelActiveBorderThickness,
	}
}
...
func (dd *DropDown) Layout(th *material.Theme, gtx C) D {
	...
	return dd.DdWidget.Layout(th, pgtx, gtx, dd)
}
