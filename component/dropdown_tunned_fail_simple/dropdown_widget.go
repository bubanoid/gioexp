...
func NewDropDownWidget(items []string, hint string, offsetX, offsetY float64, dropdownTextSize unit.Sp, selectedMenuItemHandler MenuItemHandlerType) *DropDownWidget {
	dropdownWidget := DropDownWidget{
		items:            items,
		hint:             hint, // todo (AA): check how we use hint here
		dropdownTextSize: dropdownTextSize, selectedMenuItemHandler: selectedMenuItemHandler,
	}
...
func (ddWidget *DropDownWidget) Layout(th *material.Theme, pgtx, gtx C, dd *DropDown) D {
	...
	// todo (AA): should we make this area as rounded rectangle as actual widget is?
	// Clip events to the DdWidget area for collapsed dropdown.
	...
	return layout.Stack{}.Layout(pgtx,
		layout.Stacked(func(gtx C) D {
			...
			label := ddWidget.items[ddWidget.Selected]
			// todo (AA)
			...
			// todo (AA): We need to pass ddWidget.dropdownTextSize
			//return FocusBorder(th, ddWidget.focused, dd).Layout(gtx, hint)

			return LabelBorder(th, dd).Layout(gtx, th, dd, ddWidget.focused, label, ddWidget.hint)
		}),
