...
type ContextArea struct {
	...
	// OffsetX is the offset of the dropdown along the X axis
	OffsetX float64

	// OffsetY is the offset of the dropdown along the Y axis
	OffsetY float64
}
...
func (r *ContextArea) Layout(gtx, wgtx C, w layout.Widget) D {
	//off := op.Offset(offset).Push(gtx.Ops)
	wgtx.Constraints = layout.Exact(wgtx.Constraints.Max)
	r.Update(wgtx)
	...
	if r.active {
		...
		pos := image.Point{
			X: int(math.Round(float64(r.position.X) + r.OffsetX)),
			Y: int(math.Round(float64(r.position.Y) + r.OffsetY)),
		}
		macro := op.Record(gtx.Ops)
